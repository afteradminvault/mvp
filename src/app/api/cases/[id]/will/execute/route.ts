...
/** The testator confirming they've physically signed per the jurisdiction's execution requirements — an explicit, honest user action, never inferred. */
export async function POST(_request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const executed = await service.markExecuted(will.id);
    await writeAuditLog(session.supabase, {
      estateId: id,
      actorUserId: session.userId,
      eventType: "will_executed",
      targetTable: "wills",
      targetId: executed.id,
    });
    return NextResponse.json({ will: executed });
  } catch (error) {
