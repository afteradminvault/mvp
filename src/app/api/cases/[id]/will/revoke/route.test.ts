...
beforeEach(() => {
  ...
  fakeWillRepository = {
    ...
    deleteBequest: vi.fn(),
    createVersion: vi.fn(),
    setStatus: vi.fn().mockResolvedValue(makeWill({ status: "revoked" })),
    listExecutors: vi.fn(),
  };
...
describe("POST /api/cases/:id/will/revoke", () => {
  ...
  it("returns 409 when the will is already revoked", async () => {
    fakeWillRepository.getWill = vi.fn().mockResolvedValue(makeWill({ status: "revoked" }));

    const response = await POST(new Request("http://localhost", { method: "POST" }), routeParams());
    expect(response.status).toBe(409);
  });

  it("revokes the will, writes an audit log, and returns 200", async () => {
    const response = await POST(new Request("http://localhost", { method: "POST" }), routeParams());
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body.will.status).toBe("revoked");
    expect(writeAuditLogMock).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ eventType: "will_revoked" }));
  });
});
