...
function patchRequest(body: unknown): Request {
  return new Request("http://localhost/api/cases/case-1/will/guardian", {
    method: "PATCH",
...
beforeEach(() => {
  ...
  fakeWillRepository = {
    ...
    createWill: vi.fn(),
    getWill: vi.fn(),
    updateGuardianInfo: vi.fn().mockResolvedValue(
      makeWill({ hasMinorChildren: true, guardianFullName: "Aunt Sarah", guardianRelationship: "sister" }),
    ),
...
describe("PATCH /api/cases/:id/will/guardian", () => {
  it("returns 401 when there is no session", async () => {
    requireSessionMock.mockResolvedValue({ unauthorized: NextResponse.json({ error: "nope" }, { status: 401 }) });

    const response = await PATCH(patchRequest({ hasMinorChildren: true }), routeParams());
    expect(response.status).toBe(401);
    expect(fakeWillRepository.updateGuardianInfo).not.toHaveBeenCalled();
  });
  ...
  it("updates guardian info and returns 200", async () => {
    const response = await PATCH(
      patchRequest({ hasMinorChildren: true, guardianFullName: "Aunt Sarah", guardianRelationship: "sister" }),
      routeParams(),
    );
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body.will.guardianFullName).toBe("Aunt Sarah");
    expect(fakeWillRepository.updateGuardianInfo).toHaveBeenCalledWith(
      "will-1",
      expect.objectContaining({ hasMinorChildren: true, guardianFullName: "Aunt Sarah" }),
    );
  });

  it("returns 400 (via the real service validation) when hasMinorChildren is missing", async () => {
    const response = await PATCH(patchRequest({}), routeParams());
    expect(response.status).toBe(400);
    expect(fakeWillRepository.updateGuardianInfo).not.toHaveBeenCalled();
  });
});
