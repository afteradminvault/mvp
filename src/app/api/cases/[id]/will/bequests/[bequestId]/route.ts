...
type RouteParams = { params: Promise<{ id: string; bequestId: string }> };

export async function PATCH(request: Request, { params }: RouteParams) {
  const { bequestId } = await params;
  ...
  try {
    const bequest = await service.updateBequest(bequestId, body as Record<string, unknown>);
    return NextResponse.json({ bequest });
  } catch (error) {
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  ...
  try {
    await service.deleteBequest(bequestId);
