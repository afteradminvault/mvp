...
function patchRequest(body: unknown): Request {
  return new Request("http://localhost/api/cases/case-1/will/residuary", {
    method: "PATCH",
...
beforeEach(() => {
  ...
  fakeWillRepository = {
    ...
    updateResiduaryClause: vi.fn().mockResolvedValue(makeWill({ residuaryBeneficiaryDescription: "everything to my spouse" })),
    listBequests: vi.fn(),
    createBequest: vi.fn(),
    updateBequest: vi.fn(),
    deleteBequest: vi.fn(),
...
describe("PATCH /api/cases/:id/will/residuary", () => {
  it("returns 401 when there is no session", async () => {
    ...
    expect(fakeWillRepository.updateResiduaryClause).not.toHaveBeenCalled();
  });

  it("updates the residuary clause and returns 200", async () => {
    const response = await PATCH(patchRequest({ description: "everything to my spouse" }), routeParams());
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body.will.residuaryBeneficiaryDescription).toBe("everything to my spouse");
    expect(fakeWillRepository.updateResiduaryClause).toHaveBeenCalledWith("will-1", "everything to my spouse");
  });
});
